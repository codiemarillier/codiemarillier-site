  ...
  return (
    <section className="v2-detail-section v2-light border-t border-black/10 py-16 sm:py-24">
      <div className="grid gap-8 lg:grid-cols-[12rem_1fr] lg:gap-16">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9bdbbd]">// {label}</p>
        <div>
    ...
    <section className="v2-detail-section v2-light border-t border-black/10 py-14 sm:py-20">
      <div className="mx-auto max-w-5xl">
        <div className="grid gap-5 border-b border-black/10 pb-12 md:grid-cols-[11rem_1fr] md:gap-10">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#32634f]">// The first one</p>
          <div>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#17221e] sm:text-4xl">Tuck-shop money, a dog and an early exit.</h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-[#52625b]">{v2Voice.firstBusiness}</p>
          </div>
        </div>
        ...
        <div className="divide-y divide-black/10">
          {v2FeaturedWorkStories.map((story, index) => (
            <article key={story.slug} className="grid gap-5 bg-transparent py-12 md:grid-cols-[11rem_1fr] md:gap-10">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#32634f]">0{index + 1} / {story.location}</p>
                <span className="v2-status mt-3">{story.status}</span>
              </div>
              <div>
                <h2 className="font-serif text-3xl tracking-[-0.035em] text-[#17221e] sm:text-4xl">{story.title}</h2>
                <p className="mt-5 max-w-3xl text-base leading-8 text-[#52625b]">{story.summary}</p>
                <ul className="mt-6 flex flex-wrap gap-2">{story.responsibilities.map((item) => <li key={item} className="v2-chip">{item}</li>)}</ul>
                <p className="mt-7 max-w-3xl border-l-2 border-[#c8a96a] pl-5 font-serif text-xl leading-7 text-[#27352f]">{story.lesson}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="border-t border-black/10 pt-12">
          <div className="grid gap-5 md:grid-cols-[11rem_1fr] md:gap-10">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#32634f]">// Other work</p>
            <div>
              <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#17221e] sm:text-4xl">Useful experience does not always need a grand title.</h2>
              <div className="mt-8 divide-y divide-black/10 border-y border-black/10">
                {v2SecondaryWork.map((item) => (
                  <article key={item.title} className="grid gap-2 bg-transparent py-6 sm:grid-cols-[13rem_1fr] sm:gap-8">
                    <h3 className="text-base font-medium text-[#17221e]">{item.title}</h3>
                    <p className="leading-7 text-[#52625b]">{item.text}</p>
                  </article>
                ))}
              </div>
            </div>
      ...
      <div className="divide-y divide-black/10 border-y border-black/10">
        {v2Projects.map((project, index) => (
          <article key={project.slug} className="grid gap-8 bg-transparent py-10 lg:grid-cols-[4rem_1fr]">
            <p className="font-mono text-xs text-[#32634f]">0{index + 1}</p>
            <div>
              <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#32634f]">{project.type}</p><h3 className="mt-3 font-serif text-4xl text-[#17221e] sm:text-5xl">{project.title}</h3></div><span className="v2-status">{project.status}</span></div>
              <p className="mt-7 max-w-3xl text-lg leading-8 text-[#33443c]">{project.summary}</p>
              <div className="mt-10 grid gap-8 md:grid-cols-2">
                <div><p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#32634f]">The problem</p><p className="mt-3 leading-7 text-[#52625b]">{project.problem}</p></div>
                <div><p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#32634f]">What I did</p><ul className="mt-3 space-y-2 text-[#52625b]">{project.whatIDid.map((item) => <li key={item}>— {item}</li>)}</ul></div>
              </div>
              <blockquote className="mt-9 border-l-2 border-[#c8a96a] pl-5 font-serif text-2xl leading-tight text-[#27352f]">{project.lesson}</blockquote>
              <p className="mt-7 text-xs leading-5 text-[#6a776f]">{project.note}</p>
    ...
    <>
      ...
      <PageSection label="Archive" title="The existing investment record remains intact.">
        ...
        <div className="mt-10 grid gap-3 md:grid-cols-2">
          {v2WritingGateways.map((item) => <a key={item.href} href={item.href} className="group rounded-2xl border border-white/10 p-6 transition-colors hover:border-[#77c7a2]/40"><div className="flex justify-between gap-4"><h3 className="text-xl text-white">{item.title}</h3><span className="text-[#9bdbbd]" aria-hidden="true">↗</span></div><p className="mt-4 leading-7 text-white/50">{item.text}</p></a>)}
        </div>
      </PageSection>
    </>
  );
  ...
  return (
    <>
      <PageSection label="Featured story" title={`${v2TravelStory.country}: ${v2TravelStory.title}`}>
        <figure>
          <img
            ...
            decoding="async"
          />
          <figcaption className="mt-3 font-mono text-[10px] uppercase tracking-[0.13em] text-[#6a776f]">{v2TravelImages.cambodia.caption}</figcaption>
        </figure>
      ...
      <PageSection label="France and Monaco" title="A landlocked start, then days around the docks.">
        ...
        <blockquote className="mt-10 max-w-4xl font-serif text-3xl leading-tight text-[#17221e] sm:text-5xl">“{v2Voice.france}”</blockquote>
  ...
  return (
    <>
      ...
      <PageSection label="Principles" title="Simple rules for the work in progress.">
        <div className="grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 md:grid-cols-2">{v2Principles.map((principle) => <article key={principle.title} className="bg-[#0c0f10] p-7"><h3 className="text-xl text-white">{principle.title}</h3><p className="mt-3 leading-7 text-white/50">{principle.text}</p></article>)}</div>
      </PageSection>
  ...
  return (
    <div className="v2-shell v2-detail-shell min-h-screen overflow-x-hidden bg-[#f4f1ea] text-[#17221e]">
      ...
      <main ref={mainRef} id="v2-detail-main" tabIndex={-1} className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12">
        <section className={`v2-detail-hero flex items-end text-[#edf0ec] ${page === 'work' ? 'min-h-[38vh] py-12 sm:min-h-[44vh] sm:py-14' : 'min-h-[72vh] py-20 sm:py-28'}`}>
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9bdbbd]">{meta.eyebrow}</p>
