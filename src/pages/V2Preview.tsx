...
export default function V2Preview() {
  ...
  return (
    <div className="v2-shell v2-home min-h-screen overflow-hidden bg-[#0B1F33] text-[#F7F8FA]">
      <a href="#v2-main" className="v2-skip-link" onClick={handleSkipLink}>
        Skip to content
      </a>

      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#0B1F33]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
          <a href="#top" className="font-mono text-xs font-medium uppercase tracking-[0.2em] text-white">
            CM<span className="text-[#B08D3A]">.</span>
          </a>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden rounded-full border border-[#B08D3A]/30 bg-[#B08D3A]/10 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-[#9DBBE8] sm:inline-flex">
        ...
        {menuOpen && (
          <nav ref={mobileNavRef} id="v2-mobile-nav" className="border-t border-white/10 bg-[#0B1F33] px-5 py-4 lg:hidden" aria-label="V2 mobile navigation">
        ...
        <section id="top" className="v2-hero relative flex min-h-[760px] items-end pt-32 sm:min-h-screen">
          ...
          <div className="relative z-10 mx-auto w-full max-w-[1440px] px-5 pb-16 sm:px-8 sm:pb-20 lg:px-12 lg:pb-24">
            <div className="mb-12 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.18em] text-white/65">
              <span className="h-2 w-2 rounded-full bg-[#B08D3A] shadow-[0_0_16px_#B08D3A]" />
              {v2Identity.origin} · {v2Identity.currentContext}
            </div>
            <h1 className="max-w-6xl font-serif text-[clamp(4.2rem,13vw,12rem)] font-normal leading-[0.76] tracking-[-0.07em] text-[#F7F8FA]">
              Codie<br />Marillier<span className="text-[#B08D3A]">.</span>
            </h1>
          ...
          <div className="v2-section-grid relative z-10">
            ...
            <div>
              ...
              <div className="mt-10 grid gap-8 md:grid-cols-2">
                <div><p className="v2-lead">I grew up in Harare and later went to boarding school in Marondera. Discipline, sport, close friendships and the freedom that came with structure became a lasting part of how I see the world.</p><p className="mt-6 font-serif text-2xl leading-tight text-[#D2B25F]">“{v2Voice.familyBusiness}”</p></div>
            ...
            <div>
              ...
              <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 lg:grid-cols-3">
                {v2FeaturedWorkStories.map((story, index) => (
                  <article key={story.slug} className="bg-[#102A43] p-7">
                    <div className="flex items-center justify-between gap-3"><span className="font-mono text-xs text-[#687684]">0{index + 1}</span><span className="v2-status">{story.status}</span></div>
                    <p className="mt-10 font-mono text-[10px] uppercase tracking-[0.14em] text-[#9DBBE8]">{story.location}</p>
                ...
                {featuredProjects.map((project, index) => (
                  <article key={project.slug} className="v2-project-card v2-project-card-home">
                    <div className="flex items-start justify-between gap-4">
                      <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#687684]">P—0{index + 1}</span>
                      <span className="v2-status">{project.status}</span>
                    </div>
                    <p className="mt-12 font-mono text-[10px] uppercase tracking-[0.15em] text-[#9DBBE8]">{project.type}</p>
        ...
        <section id="travel" className="v2-section v2-travel border-t border-white/10">
          <div className="v2-section-grid relative z-10">
            ...
            <div>
              ...
              <p className="font-mono text-xs uppercase tracking-[0.18em] text-[#9DBBE8]">Featured place · {v2TravelStory.country}</p>
            ...
            <div>
              ...
              <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 md:grid-cols-3">
                {recentlyRead.map((book) => (
                  <a key={book.slug} href={`/books/${book.slug}`} className="group bg-[#102A43] p-7 transition-colors hover:bg-[#153854]">
                    ...
                    <span className="mt-8 inline-block text-[#9DBBE8] transition-transform group-hover:translate-x-1" aria-hidden="true">→</span>
                  </a>
                ))}
              </div>
