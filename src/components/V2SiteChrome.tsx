...
export function V2Header({ currentSection }: { currentSection: V2NavigationSection }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0B1F33]/95 backdrop-blur-xl">
      ...
      {menuOpen && (
        <nav ref={mobileNavRef} id="v2-detail-mobile-nav" className="border-t border-white/10 bg-[#0B1F33] px-5 py-4 lg:hidden" aria-label="Website V2 mobile navigation">
          <div className="mx-auto grid max-w-[1440px] grid-cols-2 gap-2">
            ...
            <a href="/v2-preview#contact" className="rounded-lg px-3 py-3 text-sm text-[#9DBBE8]" onClick={() => setMenuOpen(false)}>Contact</a>
            <a href={v2Contact.instagramUrl} className="rounded-lg px-3 py-3 text-sm text-[#9DBBE8]" onClick={() => setMenuOpen(false)}>Instagram ↗</a>
          </div>
        </nav>
      )}
    </header>
...
export function V2Footer() {
  return (
    <footer className="v2-detail-footer mt-12 border-t border-black/10 px-5 py-10 sm:px-8 lg:px-12">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-5 text-xs text-[#687684] sm:flex-row sm:items-center sm:justify-between">
        <a href="/v2-preview" className="text-[#263442] hover:text-black">← Back to Website V2 home</a>
